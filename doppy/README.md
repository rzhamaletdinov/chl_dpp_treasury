...
- **Хунк 3 (`tokens.push`)** — продолжение того же переименования. Порядок индексов в массиве `tokens` сохраняется: `[0] = doppy`, `[1] = bnh`, `[2] = usdt`. Соответственно сохраняются и значения дневных лимитов в `maxTokenTransferPerDay`: 200 / 100 / 2000 (в 1e18) — те же, что в Cheelee, поэтому в diff'е они отображаются как контекст без `+`/`-`.
