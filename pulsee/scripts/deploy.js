...
async function main() {
  ...
  console.log(
    "\nNote: ownership of the Treasury (via proxy) is automatically " +
      "transferred to the hardcoded CUSTODY address inside `initialize`. " +
      "If CUSTODY is still address(0) (the TODO placeholder), this deploy " +
      "WILL revert with 'Ownable: new owner is the zero address'. " +
      "After a successful deploy, transfer the ProxyAdmin's ownership to " +
      "the same Pulsee multisig manually via ProxyAdmin.transferOwnership(...)."
  );
}
